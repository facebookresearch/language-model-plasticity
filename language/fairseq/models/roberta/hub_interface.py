...
class RobertaHubInterface(nn.Module):
    ...
    def encode(
        self, sentence: str, *addl_sentences, no_separator=False, out_type=str,
    ) -> torch.LongTensor:
        """
        ...
        requires leading spaces. For example::
            ...
            [0, 8331, 2]
        """
        bpe_sentence = "<s> " + self.bpe.encode(sentence, out_type=out_type) + " </s>"
        for s in addl_sentences:
            bpe_sentence += " </s>" if not no_separator else ""
            bpe_sentence += " " + self.bpe.encode(s, out_type=out_type) + " </s>"
