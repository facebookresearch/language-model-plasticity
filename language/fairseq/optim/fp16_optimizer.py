...
class FP16Optimizer(_FP16OptimizerMixin, optim.FairseqOptimizer):
    ...
    def build_optimizer(cls, cfg: DictConfig, params, **kwargs):
        ...
        #### yihong 08/23/2022 adam clear embedding state evey K steps
        if 'embed_offset' in cfg.optimizer and cfg.optimizer._name == 'adamef':
            cfg.optimizer.embed_offset = params[0].numel()
        if 'clear_embed_every_K_updates' in cfg.model and cfg.optimizer._name == 'adamef':
            cfg.optimizer.clear_embed_every_K_updates = cfg.model.clear_embed_every_K_updates
        ###################################################
