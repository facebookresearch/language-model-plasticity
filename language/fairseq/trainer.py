...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                ) # Yihong: can be buggy when transfering from 1 language to another language
    ...
    def lr_step_update(self):
        ...
        if self.cfg.optimizer._name == 'adamef':
            ######### reset the lr scheduler for the token embedding ######
            body_num_updates = self.get_num_updates()
            K = self.cfg.model.clear_embed_every_K_updates
            tot = self.cfg.optimization['max_update']
            speed = tot // K 
            # emb_num_updates = body_num_updates % K
            emb_num_updates = (body_num_updates * speed) % tot
            new_lr_emb = self.lr_scheduler.step_update(emb_num_updates)
            self.optimizer.optimizer.set_lr_emb(new_lr_emb)
