...
class BaseProgressBar(object):
    ...
    def watch_param(self, model=None, step=None):
        """Watch model param histgram"""
        pass
...
def log_slurm_config():
    if 'SLURM_JOB_ID' in os.environ:
        wandb.config.update({'SLURM_JOB_ID': os.environ['SLURM_JOB_ID']})
    if 'SLURM_ARRAY_JOB_ID' in os.environ:
        wandb.config.update({'SLURM_ARRAY_JOB_ID': os.environ['SLURM_ARRAY_JOB_ID']})
    if 'SLURM_ARRAY_TASK_ID' in os.environ:
        wandb.config.update({'SLURM_ARRAY_TASK_ID': os.environ['SLURM_ARRAY_TASK_ID']})
...
class WandBProgressBarWrapper(BaseProgressBar):
    ...
    def watch_param(self, model=None, step=None):
        """Log histgram of params TODO: extend config of wandb to select which parts of parameters to track"""
        if wandb is None:
            return
        hist = wandb.Histogram(model.encoder.sentence_encoder.embed_tokens.weight.data.cpu())
        wandb.log({'params/embed_tokens': hist})
        hist = wandb.Histogram(model.encoder.lm_head.weight.data.cpu())
        wandb.log({'params/lm_head': hist})
