                ...
                with open(f'{tmp}/{split}', mode='w', encoding='utf-8', errors='surrogateescape') as fout:
                    ...
                    for i, line in enumerate(encoded_lines, start=1):
                        ...
                        if i > max_lines:
                            ...
                            pool.terminate()
                            break
            if max_lines <= 200000000:
                # Hack for count token freq
                def flatten(lst):
                    flat_list = []
                    for element in lst:
                ...
                with open(f'{tmp}/{split}', mode='r', encoding='utf-8', errors='surrogateescape') as ffreq:
                    text = ffreq.read()
                ...
                with open(f'{tmp}/{split}.freq', mode='w', encoding='utf-8', errors='surrogateescape') as ffreq_out:
                    for k,v in frequency:
                        print(k, v, file=ffreq_out)
            #### Duplicate fout for 1/ratio times
            if ratio < 1:
                copy_file = f'{tmp}/{split}' + 'copy'
                shutil.copyfile(f'{tmp}/{split}', copy_file)
                copy_times = int(1/ratio) - 1
                print(copy_times)
                print('wow')
                lang = spm_model_path.split('/spm/')[0].split('/')[-1] 
                # if lang in ['sw'] and 'wiki' in spm_model_path: # for wiki these languages are very small, duplicate it so that the training won't be slow down by very small epoch
                #     copy_times = copy_times * 100
                # if lang in ['ur'] and 'wiki' in spm_model_path:
                #     copy_times = copy_times * 25
